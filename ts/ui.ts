...
export function create100coins() {
    ...
    homeContainer?.addEventListener("click", function () {
        ...
        if (infoContainer.className === "Moreinfo") {
            ...
            getAjax(
                link,
                CreateMoreInfoFunction(infoContainer)
            );
...
export function CreateMoreInfoFunction(infoContainer: any) {

    let CoinData = this.response;
    let moreInfoELEMENT: any = document.getElementById(CoinData.id);
